...
module Admin
  class SummaryController < AdminController
    ...
    def start_date
      params[:start_date].to_i.positive? ? params[:start_date] : nil
    end

    def end_date
      params[:end_date].to_i.positive? ? params[:end_date] : nil
    end

    def collection_id
      params[:collection_id].to_i.positive? ? params[:collection_id].to_i : nil
    end

    def institution_id
      params[:institution_id].to_i.positive? ? params[:institution_id].to_i : nil
    end
  end
end
