...
module Admin
  module Cms
    class TranscriptsController < AdminController
      # Deals with publishing, unpublishing, and deleting multiple transcripts.
      include UpdateMultipleTranscriptsConcern

      before_action :set_transcript, only: %i[edit update destroy reset_transcript sync]
      ...
      def sync
        Azure::SpeechToTextJob.perform_later @transcript.id if @transcript.azure?
        @transcript.reload
      end
