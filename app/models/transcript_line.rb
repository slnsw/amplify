...
class TranscriptLine < ApplicationRecord
  has_paper_trail
  include PgSearch
  multisearchable against: %i[original_text guess_text]
  pg_search_scope :search_by_all_text, against: %i[guess_text original_text] # User text weighted more than original text
  ...
  def recalculate(edits = nil, project = nil)
    ...
    if edits.length.positive?
      edits_filtered = edits.select { |edit| edit[:text].present? && edit[:text] != original_text || edit[:user_hiearchy] >= consensus['superUserHiearchy'] }
    end
    ...
    if edits_filtered.length <= 0 && edits.length >= consensus['minLinesForConsensusNoEdits']
      edits_filtered = edits.select { |_edit| true }
    end

    # Set best guess text
    best_edit = chooseBestEdit(edits_filtered, project)
    best_guess_text = best_edit[:edit][:text] unless best_edit.nil? || best_edit[:edit].nil?
    ...
    if status_id <= 1 && edits_filtered.length.positive?
    ...
    if status_changed || best_guess_text != guess_text
      ...
      transcript.delta(old_status_id, status_id, statuses) if status_changed
    end
  ...
  def recalculateSpeaker(edits = nil, project = nil)
    ...
    if edits.length.positive?
      ...
      edits = edits_priority.select { |_edit| true } if edits_priority.length.positive?
    end

    if edits.length.positive?
      # Group the edits by speaker_id
      groups = edits.group_by(&:speaker_id)
  ...
  def chooseBestEdit(edits, project)
    ...
    if edits.length.positive?
      edits_priority = edits.select { |edit| edit[:user_hiearchy] >= consensus['superUserHiearchy'] }
      if edits_priority.length.positive?
        edits_priority = edits_priority.sort_by { |edit| [edit[:user_hiearchy] * -1, Time.zone.now - edit.updated_at] }
        edits = edits_priority.select { |_edit| true }
    ...
    best_group = { text: edits[0].normalizedText, count: 1 } if edits.length.positive?
    best_edit = edits[0] if edits.length.positive?

    if edits.length > 1 && edits_priority.blank?
      # Group the edits by normalized text
      groups = edits.group_by(&:normalizedText)
      # Convert groups from hash to array
      ...
      best_edits = edits.select { |_edit| true }
      ...
      # Sort the edits
      best_edits = best_edits.sort_by do |edit|
        score = 0
